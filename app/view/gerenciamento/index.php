<!DOCTYPE html>
<html lang="en">

<head>
    <?php require_once 'app/view/structure/head.php' ?>

    <title>Home - Gerenciamento</title>
</head>

<body>
    <section class="container" style="height: 100vh">
        <h1 class="p-5 d-flex justify-content-center">
            Gerenciamento
        </h1>
        <a href="/bruxasburger/gerenciamento/filiais" class="btn btn-success w-100 mb-3">Filiais</a>
        <a href="/bruxasburger/gerenciamento/cardapio" class="btn primary w-100">Cardápio</a>
    </section>

    <?php require_once 'app/view/structure/footer.php' ?>
</body>

</html>